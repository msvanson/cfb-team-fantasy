...
const officialConferenceByTeam=new Map((memberships||[]).map(x=>[x.team_id,x.official_conference]));
 ...
 for(const [code,winnerType,reachType] of confs){
   const win=marketProbabilities(grouped(winnerType),1);
   const reach=reachType?marketProbabilities(grouped(reachType),2):new Map();
   ...
   const usableReachTotal=members.reduce((sum,[sdid])=>sum+(reach.get(sdid)||0),0);
   const useReach=Boolean(reachType)&&usableReachTotal>=1.5;
   for(const [sdid] of members){
     const wp=win.get(sdid)||0;
     const rp=reach.get(sdid)||0;
