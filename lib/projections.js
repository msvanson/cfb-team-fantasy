...
export async function refreshSeasonProjections(){
 ...
 if(payload.length!==138){
   const payloadIds=new Set(payload.map(x=>x.team_id));
   const missingPayload=teams
     .filter(t=>!payloadIds.has(t.team_id))
     .map(t=>{
       const win=winByTeam.get(t.team_id);
       const espnEntry=[...espnToLeague.entries()].find(([,mapped])=>mapped.team_id===t.team_id);
       return `${t.school} [winTotal=${win?'yes':'NO'}, espn=${espnEntry?'yes':'NO'}]`;
     });
   throw new Error(`Projection payload incomplete: ${payload.length}/138; missing: ${missingPayload.join(', ')}`);
 }
