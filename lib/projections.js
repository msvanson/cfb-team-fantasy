...
export async function refreshSeasonProjections(){
 ...
 // ESPN team id mapping via season teams endpoint.
 const ti=await json('https://sports.core.api.espn.com/v2/sports/football/leagues/college-football/seasons/2026/teams?limit=500&lang=en&region=us');
 const espn=[];
 for(const it of (ti.items||[])){const t=await json(it.$ref);espn.push({id:String(t.id),name:t.displayName||t.name||t.nickname||''})}
 const byLeague=new Map(teams.map(t=>[norm(t.school),t]));
 function matchLeague(name){let n=norm(name);const opts=[n,alias[n]].filter(Boolean);for(const q of opts){if(byLeague.has(q))return byLeague.get(q)}
  for(const [ln,t] of byLeague){if(n.length>4&&(ln.startsWith(n+' ')||n.startsWith(ln+' ')))return t}return null}
 const espnToLeague=new Map();
 const usedLeagueTeamIds=new Set();
 for(const e of espn){
   const t=matchLeague(e.name);
   // ESPN's season team endpoint contains additional non-FBS programs.
   // Keep exactly one ESPN mapping per team in our 138-team league directory.
   if(t&&!usedLeagueTeamIds.has(t.team_id)){
     espnToLeague.set(e.id,t);
     usedLeagueTeamIds.add(t.team_id);
   }
 }
 const winByTeam=new Map();for(const w of winRows){const t=matchLeague(w.team);if(t)winByTeam.set(t.team_id,w)}
 // NDSU only: retain prior real captured value if present; otherwise require manual baseline env var.
 const ndsu=teams.find(t=>/north dakota state/i.test(t.school));
 if(ndsu&&!winByTeam.has(ndsu.team_id)){
   const manual=Number(process.env.NDSU_WIN_TOTAL);
   if(Number.isFinite(manual))winByTeam.set(ndsu.team_id,{team:ndsu.school,line:manual,over:-110,under:-110,wins:manual,source:'manual'});
 }
 if(winByTeam.size<138)throw new Error(`Real win-total coverage incomplete: ${winByTeam.size}/138. Set NDSU_WIN_TOTAL for North Dakota State.`);

 if(espnToLeague.size!==138)throw new Error(`ESPN FBS mapping incomplete: ${espnToLeague.size}/138`);
 ...
 const {data:run,error:re}=await supabase.from('team_projection_runs').insert({season_id:1,source:'espn_sbd_real',is_test:false,notes:'ESPN DraftKings futures + SportsBettingDime win totals'}).select('id').single();if(re)throw re;
 const rows=payload.map(x=>({...x,run_id:run.id}));
