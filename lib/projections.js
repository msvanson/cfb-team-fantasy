...
export async function refreshSeasonProjections(){
 ...
 if(espnToLeague.size!==138){
   const miamiCandidates=teams
     .filter(t=>norm(t.school).includes('miami'))
     .map(t=>`${t.school} [${t.conference_code}]`)
     .join(', ');
   throw new Error(
     `ESPN title-market mapping incomplete: ${espnToLeague.size}/138; ` +
     `unmatched: ${espnUnmapped.join(', ')}; ` +
     `database Miami teams: ${miamiCandidates}`
   );
 }
