...
function normalizeMap(raw,target,slots=1){
 const entries=[...raw.entries()].filter(([id])=>target.has(String(id)));
 const total=entries.reduce((s,[,p])=>s+p,0);
 if(!total)return new Map();
 const scale=slots/total;
 return new Map(entries.map(([id,p])=>[String(id),Math.min(1,p*scale)]));
}
function marketMap(markets,label){
 const m=markets.find(x=>(x.displayName||x.name)===label);
 const out=new Map();
 for(const f of (m?.futures||[])){
   for(const b of (f?.books||[])){
     const id=String(b?.team?.$ref||'').match(/\/teams\/([^/?]+)/)?.[1];
     const odds=b?.value??b?.odds?.value??b?.odds??null;
     if(id&&odds!=null)out.set(id,prob(odds));
   }
 }
 return out;
}
export async function refreshSeasonProjections(){
 if(!process.env.SUPABASE_SERVICE_ROLE_KEY)throw new Error('SUPABASE_SERVICE_ROLE_KEY is not configured on the server');
 ...
 const winRows=parseWinTotals(await sr.text());
  ...
  // ESPN's championship market can shrink during the season as teams are
 // eliminated or removed by the sportsbook. Supplement the current market
 // with the verified ESPN IDs from the most recent successful projection run.
 ...
 if(priorRun?.id){
   const {data:priorRows,error:priorRowsError}=await supabase
     .from('team_projection_snapshots')
     .select('components')
   ...
   priorIds=[...new Set(
     (priorRows||[])
       .map(row=>String(row?.components?.espnTeamId||''))
       .filter(Boolean)
   )];
 }

 const universeIds=[...new Set([...titleIds,...priorIds])];
 if(universeIds.length!==138){
   throw new Error(
     `ESPN team universe incomplete: ${universeIds.length}/138 `+
     `(current market ${titleIds.length}, prior snapshot ${priorIds.length})`
   );
 }
 ...
 if(ndsu&&!winByTeam.has(ndsu.team_id)){
      const configured=Number(process.env.NDSU_WIN_TOTAL);
   const manual=Number.isFinite(configured)?configured:9;
 ...
 const champ=normalizeMap(champRaw,fbsIds,1);
 const sf=normalizeMap(sfRaw,fbsIds,4);
 const ncg=normalizeMap(ncgRaw,fbsIds,2);
 ...
 for(const [code,label] of Object.entries(confMarket)){
   ...
   const cm=normalizeMap(marketMap(markets,label),eligible,1);
 ...
 const pac=normalizeMap(champRaw,pacIds,1);for(const [id,p] of pac)confPoints.set(id,6*p);
 ...
 // ESPN provides "reach semifinal" odds, not direct "make CFP" odds.
 // Use those odds as relative weights and normalize them to exactly 12 expected CFP entrants.
 // This preserves the market ranking while guaranteeing the league's 24-point CFP appearance pool.
 const sfWeightTotal=[...sf.values()].reduce((sum,p)=>sum+p,0);
 const sf12=new Map([...sf.entries()].map(([id,p])=>[id,sfWeightTotal?12*p/sfWeightTotal:0]));
 const winWeights=new Map();
 let winWeightTotal=0;
 for(const eid of fbsIds){
   const weight=(sf.get(eid)||0)+(ncg.get(eid)||0)+(champ.get(eid)||0);
   winWeights.set(eid,weight);
   winWeightTotal+=weight;
 }
 ...
 for(const [eid,t] of espnToLeague){
   ...
   const ncp=ncg.get(eid)||0,chp=champ.get(eid)||0;
   const cfpWins=winWeightTotal>0?11*(winWeights.get(eid)||0)/winWeightTotal:0;
   const ncgPts=ncp,titlePts=chp;
 ...
 const {error:ie}=await supabase.from('team_projection_snapshots').insert(rows);if(ie)throw ie;
   const [{data:ownerTotals,error:ote},{data:owners,error:oe}]=await Promise.all([
   supabase.from('owner_projection_totals').select('owner_id,projected_points').eq('season_id',1),
   supabase.from('owners').select('id,draft_slot').eq('season_id',1)
 ]);
 if(ote)throw ote;
 if(oe)throw oe;
 ...
 const draftSlots=new Map(
   (owners||[]).map(owner=>[
     Number(owner.id),
     Number(owner.draft_slot)
   ])
 );

 const ranked=(ownerTotals||[]).sort((a,b)=>
   Number(b.projected_points)-Number(a.projected_points)||
   (draftSlots.get(Number(a.owner_id))??999)-
   (draftSlots.get(Number(b.owner_id))??999)
 );

 const ownerRows=ranked.map((owner,index)=>({
   run_id:run.id,
   season_id:1,
   owner_id:owner.owner_id,
   projected_points:owner.projected_points,
   projected_rank:index+1,
   snapshot_at:run.snapshot_at
 }));

 const {error:opie}=await supabase
   .from('owner_projection_snapshots')
   .upsert(ownerRows,{onConflict:'run_id,owner_id'});
