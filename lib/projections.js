...
const alias={"app state":"appalachian state","hawaii":"hawaii","san jose state":"san jose state","ul monroe":"ulm","florida international":"fiu","miami ohio":"miami (oh)","miami (ohio)":"miami (oh)","nc state":"nc state"};
...
const confs=[['ACC','ACC Winner','To Reach ACC Final'],['AAC','American Conference Winner','To Reach The American Conference Final'],['Big 12','Big 12 Winner','To Reach Big 12 Conference Final'],['Big Ten','Big 10 Winner','To Reach Big 10 Conference Final'],['CUSA','C-USA Winner','To Reach C-USA Final'],['MAC','MAC Winner','To Reach MAC Conference Final'],['MWC','MWC Winner','To Reach Mountain West Conference Final'],['SEC','SEC Winner','To Reach SEC Conference Final'],['Sun Belt','Sun Belt Winner',null],['Pac-12','PAC-12 Winner','To Reach Pac-12 Conference Final']];
export async function runProjectionImport(){const key=process.env.SPORTSDATAIO_API_KEY;if(!key)throw new Error('SPORTSDATAIO_API_KEY is missing');const admin=process.env.CFB_ADMIN_SECRET;if(!admin)throw new Error('CFB_ADMIN_SECRET is missing');const [events,sdTeams,{data:league,error:le}]=await Promise.all([getJson(`${ODDS}/BettingFuturesBySeason/2026`,key),getJson(`${SCORES}/Teams`,key),supabase.from('team_directory').select('*').eq('season_id',1)]);if(le)throw le;
 ...
 const confPoints=new Map();for(const [code,winnerType,reachType] of confs){const win=marketProbabilities(grouped(winnerType),1);const reach=reachType?marketProbabilities(grouped(reachType),2):new Map();for(const [sdid,t] of sdToInternal){if(t.conference_code!==code)continue;const wp=win.get(sdid)||0;const rp=reachType?(reach.get(sdid)||0):0;confPoints.set(sdid,reachType?(rp+4*wp):(6*wp))}}
