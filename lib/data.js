...
export async function getAllStandings(){const {data,error}=await supabase.from('owner_standings').select('*').order('season_id').order('rank').order('draft_slot');if(error)throw error;return data||[]}