...
export async function getOwnerCurrentTeamContributions(){const {data,error}=await supabase.from('owner_current_team_contributions').select('*').eq('season_id',1).order('conference_display_order').order('school');if(error)throw error;return data||[]}
export async function getScheduleTeams(){const {data,error}=await supabase.from('teams').select('id,school,abbreviation,mascot');if(error)throw error;return (data||[]).map(t=>({lookup_id:t.id,school:t.school,abbreviation:t.abbreviation,mascot:t.mascot}))}
...
export async function getOwnerProjectionHistory(){const {data,error}=await supabase.from('owner_projection_snapshots').select('*').eq('season_id',1).order('snapshot_at').order('projected_rank');if(error)throw error;return data||[]}
...
export async function getPublishedWeeklyRecaps(){const {data,error}=await supabase.from('weekly_recaps').select('*').eq('season_id',1).eq('status','published').order('source_finalized_at',{ascending:false});if(error)throw error;return data||[]}
...
export async function getTeamProjectionHistory(teamId){const {data,error}=await supabase.from('team_projection_history').select('*').eq('season_id',1).eq('team_id',teamId).order('snapshot_at');if(error)throw error;return data||[]}

export async function getTeamTransactions(teamId){const {data,error}=await supabase.from('waiver_transaction_history').select('*').eq('season_id',1).or(`add_team_id.eq.${teamId},drop_team_id.eq.${teamId}`).order('effective_at',{ascending:false});if(error)throw error;return data||[]}
