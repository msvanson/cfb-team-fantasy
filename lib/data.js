...
export async function getOwner(ownerId){const {data,error}=await supabase.from('owner_standings').select('*').eq('season_id',1).eq('owner_id',ownerId).maybeSingle();if(error)throw error;return data}
...
export async function getOwnerEvents(ownerId){const {data,error}=await supabase.from('scoring_events').select('*').eq('season_id',1).eq('owner_id',ownerId).order('created_at',{ascending:false});if(error)throw error;return data||[]}
