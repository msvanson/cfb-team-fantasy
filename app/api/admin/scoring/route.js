...
export async function POST(req){
  ...
  try{
    ...
    await adminRpc('admin_log_action',{p_action_type:'scoring_adjustment',p_summary:`Manual scoring adjustment: ${b.points} points`,p_details:{teamId:b.teamId,eventType:b.eventType,weekKey:b.weekKey,note:b.note}});return NextResponse.json({ok:true,import { NextResponse } from 'next/server';
import { isAdminAuthenticated } from '../../../../lib/admin-auth';
import { adminRpc } from '../../../../lib/admin-rpc';
export async function POST(req){
  if(!await isAdminAuthenticated()) return NextResponse.json({ok:false,error:'Unauthorized'},{status:401});
