...
export async function POST(req){
 ...
 const { data: teams, error: teamsError } = await s
  .from('team_directory')
  .select('*')
  .eq('season_id', 1);
...
if (teamsError) {
 ...
 const { data: max, error: priorityError } = await s
  .from('waiver_claims')
  .select('priority')
  .eq('season_id', 1)
  .eq('owner_id', ownerId)
...
if (priorityError) {
  return NextResponse.json(
    { ok: false, error: 'Unable to submit waiver claim right now' },
    { status: 500 }
  );
}
...
export async function DELETE(req){
 ...
 return NextResponse.json(
  error
    ? { ok: false, error: 'Unable to cancel waiver claim right now' }
    : { ok: true },
...
export async function PATCH(req){
 ...
 if (error) {
  return NextResponse.json(
    { ok: false, error: 'Unable to reorder waiver claims right now' },
    { status: 400 }
  );
}
